$(function () {
    ...
    function getListData() {
        var url = baseUrlFromAPP + "/getPM2AndPM10";
        var params = {
            fSubid: subidFromAPP
        };
        getData(url, params, function (data) {
            if (data.PM2AndPM10ObList != null) {
                if (data.PM2AndPM10ObList.length > 0) {
                    $("#cardList").empty();
                    $(data.PM2AndPM10ObList).each(function () {
                        var tempVal = "--";
                        var humiVal = "--";
                        if (this.fPM2 != undefined && this.fPM2 != null) {
                            tempVal = parseFloat(this.fPM2).toFixed(1);
                        }
                        ...
                        $("#cardList").append('<section class="sectionCard" value="' + this.F_MeterCode + '">' +
                            ...
                            '<p>' + Operation['ui_PM2'] + ':' + tempVal + this.fPM2Unit + '</p>' +
                            '<p>' + Operation['ui_PM10'] + ':' + humiVal + this.fPM10Unit + '</p></section>');
                    });
    ...
    function getChartData() {
        ...
        getData(url, params, function (data) {
            if (data.FPM2AndPM10ByDate != null) {
                if (data.FPM2AndPM10ByDate.length > 0) {
                    $(data.FPM2AndPM10ByDate).each(function () {
                        time.push(this.fCollecttime.substring(11, 16));
                        temp.push(this.fPM2);
                        humi.push(this.fPM10);
                    });
                }
            }
            chartData = {
