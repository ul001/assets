class MobileCalendar {
  ...
  defaultValueInit() {
    ...
    //初始化年插件默认值
    else if (/^\d{4}$/.test(this.trigger.value)) {
      let [yy] = this.trigger.value.match(/(^|-)\d{1,4}/g);
      dateArr.yy = yy - this.minY;
    }
    //初始化时分插件默认值
    ...
    if (this.gearDate.querySelector(".date_yy")) {
      ...
      if (this.gearDate.querySelector(".date_mm")) {
  ...
  setDateGearTooth() {
    ...
    if (date_dd && date_dd.getAttribute("val")) {
      ...
      if(ddVal+1>maxMonthDays){
        ddVal = maxMonthDays-1;
      }
      //p 当前节点前后需要展示的节点个数
  ...
  bindEvent(type) {
    function getTemp(containsStr, str) {
      let tempStr = `<div class="date_ctrl slideInUp">
            ...
            <div class="date_btn lcalendar_cancel">${Operation.ui_cancel}</div>
            <div class="date_btn lcalendar_finish">${Operation.ui_confirm}</div>
            </div>
    ...
    let ystr = `<div>
        ...
        <div class="date_grid">
        <div>${Operation.ui_yy}</div>
        </div>
        </div>`;
    let mstr = `<div>
        <div class="gear date_mm" data-datetype="date_mm"></div>
        <div class="date_grid">
        <div>${Operation.ui_mm}</div>
        </div>
        </div>`;
    let dstr = `<div>
        <div class="gear date_dd" data-datetype="date_dd"></div>
        <div class="date_grid">
        <div>${Operation.ui_dd}</div>
        </div>
        </div>`;
    let hstr = `<div>
        <div class="gear time_hh" data-datetype="time_hh"></div>
        <div class="date_grid">
        <div>${Operation.ui_hh}</div>
        </div>
        </div>`;
    let mistr = `<div>
        <div class="gear time_mm" data-datetype="time_mm"></div>
        <div class="date_grid">
        <div>${Operation.ui_min}</div>
        </div>
        </div>`;
    this.trigger.addEventListener(
      "click",
      {
        y: this.popup("gearDate", getTemp(`${ystr}`, "y_roll")), //呼出年月插件
