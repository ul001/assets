$(function () {
    ...
    var tokenFromAPP =
        "eyJ0eXAiOiJKV1QiLCJhbGciOiJIUzI1NiJ9.eyJleHAiOjE1ODgyMjkwNTksInVzZXJuYW1lIjoiaGFoYWhhIn0.0rPtex1A_IXCvgvGqb6XNLBrZJaVJCl-lYPxbRJsxq0";
    ...
    function getListData() {
        var url = baseUrlFromAPP + "/getFPartialDischarge";
        var params = {
            fSubid: subidFromAPP
        };
        getData(url, params, function (data) {
            if (data.FPartialDischargeList != null) {
                if (data.FPartialDischargeList.length > 0) {
                    $("#cardList").empty();
                    $(data.FPartialDischargeList).each(function () {
                        var partialDischargeVal = "--";
                        if (
                            this.partialDischarge != undefined &&
                        ...
                        $("#cardList").append(
                            ...
                            this.f_MeterName +
                            "</p>" +
                            '<img src="image/DischargePic.png"/>' +
                            "<p>" +
                            Operation["ui_discharge"] +
                            ":" +
                            partialDischargeVal +
                            this.partialDischargeUnit +
                            "</p></section>"
                        );
                    });
    ...
    function getChartData() {
        ...
        getData(url, params, function (data) {
            // FPartialDischargeByDate
            // FPartialDischargeList
            if (data.FPartialDischargeByDate != null) {
                if (data.FPartialDischargeByDate.length > 0) {
                    $(data.FPartialDischargeByDate).each(function () {
                        time.push(this.fCollecttime.substring(11, 16));
                        discharge.push(this.fPartialDischarge);
                    });
                }
            }
            chartData = {
