...
$(function () {
    ...
    function getListData() {
        var url = baseUrlFromAPP + "/getVSpeed";
        var params = {
            fSubid: subidFromAPP
        };
        getData(url, params, function (data) {
            if (data.VSpeedObList != null) {
                if (data.VSpeedObList.length > 0) {
                    ...
                    $(data.VSpeedObList).each(function () {
                        ...
                        if (this.fVSpeed != undefined && this.fVSpeed != null) {
                            speedVal = parseFloat(this.fVSpeed).toFixed(1);
                        }
                        $("#cardList").append('<section class="sectionCard" value="' + this.F_MeterCode + '">' +
                            '<p>' + meterName + '</p>' +
                            '<img src="image/Vibration.png"/>' +
                            '<p>' + Operation['ui_Vibration'] + ':' + speedVal + this.fVSpeedUnit + '</p></section>');
                    });
    ...
    function getChartData() {
        ...
        getData(url, params, function (data) {
            if (data.FVSpeedByDate != null) {
                if (data.FVSpeedByDate.length > 0) {
                    $(data.FVSpeedByDate).each(function () {
                        time.push(this.fCollecttime.substring(11, 16));
                        noise.push(this.fVSpeed);
                    });
                }
            }
            chartData = {
